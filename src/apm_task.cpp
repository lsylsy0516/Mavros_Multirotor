...
bool Multirotor::takeoff()
{
    ...
    takeoff.request.altitude = task_points[0][2];
    if(current_state.armed){    // is armed
...
void Multirotor::init()
{
    ...
    nh.param<float>("fly_height", fly_height, 5);
    ...
    task_points.push_back(Eigen::Vector3d(drone_pos[0],drone_pos[1]+30,fly_height)); // task point 1
    task_points.push_back(Eigen::Vector3d(drone_pos[0]-4,drone_pos[1]+30,fly_height)); // task point 2
...
void Multirotor::run()
{
    ...
    while (switchflag == 0)
    {
        ...
        {
            ROS_INFO("distance2d :%f", distance2d);
        }
        rate.sleep();
        ros::spinOnce();
    }
